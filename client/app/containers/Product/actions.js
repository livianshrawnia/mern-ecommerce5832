...
export const fetchProducts = (filter, slug) => {
  return async (dispatch, getState) => {
    ...
    try {
      const response = await axios.post(`/api/web/product/list`, {});
...
export const fetchMerchantProducts = (filter, slug) => {
  return async (dispatch, getState) => {
    dispatch({ type: SET_PRODUCTS_LOADING, payload: true });

    try {
      const response = await axios.get(`/api/mer/product/list`);
...
export const fetchBrandProducts = slug => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      const response = await axios.post(`/api/web/product/list`, payload);
...
export const fetchProduct = slug => {
  return async (dispatch, getState) => {
    ...
    try {
      const response = await axios.get(`/api/web/product/get/${slug}`);

      const inventory = response.data.result.product.quantity;
      const product = { ...response.data.result.product, inventory };
...
export const fetchProductsSelect = () => {
  return async (dispatch, getState) => {
    try {
      ...
      let formattedProducts = formatSelectOptions(response.data.result.products, true);
...
export const deleteProduct = (id, index) => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.delete(`/api/mer/product/delete/${id}`);
...
export const addProduct = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const categories = getState().category.selectedCategories.value;
      ...
      const { isValid, errors } = allFieldsValidation(newProduct, rules, {
        ...
        'required.categories': 'Category is required.'
      });
      ...
      const response = await axios.post(`/api/mer/product/add`, newProduct);
      ...
      if (response.data.error === false) {
        ...
        dispatch({
          type: ADD_PRODUCT,
          payload: response.data.result.product
        });
