...
export const fetchOrders = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`/api/web/order/list`);

      if (response.data.result.orderList) {
        dispatch({
          type: FETCH_ORDERS,
...
export const fetchOrder = id => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`/api/web/order/get/${id}`);

      dispatch({
        type: FETCH_ORDER,
        payload: response.data.result.order
      });
    } catch (error) {
      handleError(error, dispatch);
    } finally {
...
export const cancelOrder = () => {
  return async (dispatch, getState) => {
    try {
      ...
      await axios.delete(`/api/web/order/cancel/${order._id}`);
